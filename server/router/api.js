...
// @ts-ignore
const loadArticles = async () => new Map(await fs.promises.readdir('./content').then(articles => articles.map(article => {
...
export default function api(fastify, opts, done) {
  ...
  fastify.get('/articles/:article/content', (req, res) => {
    ...
    // @ts-ignore
    if (req.query.raw || req.headers['accept'].includes('text/plain') || req.headers['accept'].includes('text/markdown')) {
      res.header('content-type', 'text/plain; charset=utf-8').send(article.content);
    } else {
      res.header('content-type', 'text/html; charset=utf-8').send(article.parsed);
    }
  });
